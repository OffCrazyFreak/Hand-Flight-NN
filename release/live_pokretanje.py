...
# --- PARAMETERS -----
model_weights_path = "/Users/tomislavmatanovic/Documents/FER/NEUMRE/Projekt/KutInterfer/weights_test_model_val_0.43_240.weights.h5"
shared_path = "/Users/tomislavmatanovic/Documents/FER/NEUMRE/Projekt/igra/shared.txt"
bbox_format = "center"
...
def downscale_image(frame, input_width, input_height):
    ...
    if (input_width/input_height) == 16/9:
        ...
        # Crop the image
        cropped_frame = frame[y_offset:y_offset+target_height, x_offset:x_offset+target_width]
        return cv2.resize(cropped_frame, (360, 240), interpolation=cv2.INTER_LINEAR)
        ...
        print(f"Unsupported format -> Dimensions = {input_width}x{input_height} but has to be either 3:2 or 16:9!")
        return None
...
def write_to_file(class_name, x_center, y_center, angle, GAME_GRID = GAME_GRID):
    ...
    with open(shared_path, 'w') as file:
        ...
        try:
            ...
            # Always unlock the file after writing
...
nn_model.load_weights(model_weights_path)
...
def update_raw_predictions(new_class, new_x, new_y, new_angle):
    ...
    with lock:
        curr_class, target_x, target_y, target_angle = new_class, new_x, new_y, new_angle
...
try:
    ...
    while True:
        ...
        if Verbose_log:
            print(f"Camera FPS: {cap.get(cv2.CAP_PROP_FPS):.2f}, time taken: {(1/cap.get(cv2.CAP_PROP_FPS)):.5f}s")
        ...
        most_common_class = [i for i in set(class_history) if class_history.count(i) == max(class_count)]
        ...
        if pred_class != 1 and curr_class !=1:
            ...
            if curr_class == 2:
                update_raw_predictions(2, pred_output[0][4], pred_output[0][5], last_goog_angle_deg)
            elif (pred_class != 2):
                update_raw_predictions(pred_class, pred_output[0][4], pred_output[0][5], last_goog_angle_deg)


        # Display the image on Matplotlib
        wait_t = int((1000/interference_FPS)-130-(3*prep_t*1000)-(prediction_time*1000)-(angle_goog_time*1000)-(write_time*1000)) / 1000 # in seconds # još i model predict time
        if Verbose_log:
        ...
        if VISUALIZE_IMAGES:
            ...
            ax.text(10, 10, f'Class: {pred_class}, angl: {last_goog_angle_deg}', color='white', fontsize=12, ha='left', va='top')
        ...
        aprox_max_time = (100+(2.5*prep_t*1000)+(prediction_time*1000)+(angle_goog_time*1000)+(write_time*1000))/1000
        if Verbose_log:
            print(f"Game FPS: {(1 / elapsed_time):.2f}, time taken: {elapsed_time:.5f}s")
            print(f"MAX game FPS: {(1 / aprox_max_time):.2f}, time taken: {aprox_max_time:.5f}s") # when no ploting img
            print("--------------")
            print()
